#!/usr/bin/env python3
"""
Extract Right Half from PDF Pages
================================

This script:
1. Converts PDF pages to images
2. Crops the right half of each page image
3. Saves the right half images

This gives us the actual right half of the PDF layout,
not HTML text content.
"""
...
class PDFRightHalfExtractor:
    ...
    def extract_right_half_from_image(self, image_path, output_path):
        """Extract right half from an image"""
        try:
            with Image.open(image_path) as img:
                ...
                right_half.save(output_path, 'PNG', quality=95, optimize=True)
                return True
        ...
        try:
            ...
            for page_num in range(total_pages):
                try:
                    ...
                    # Extract right half
                    output_image_path = book_output_dir / f"{book_name}_page_{page_num + 1:03d}_right.png"
                    
                    if self.extract_right_half_from_image(temp_image_path, output_image_path):
                        ...
                        print(f"   ❌ Page {page_num + 1:3d}: Failed to extract right half")
                        page_failed += 1
                        self.failed_count += 1
    ...
    def process_all_files(self):
        ...
        for book_dir in sorted(self.output_dir.iterdir()):
            if book_dir.is_dir():
                page_count = len(list(book_dir.glob("*_right.png")))
                total_images += page_count
                print(f"   📖 {book_dir.name}: {page_count} pages")
        
        print(f"\n🎉 Total right-half images generated: {total_images}")

def main():
    """Main function"""
    data_dir = "/root/sarvam/akshar-experiments-pipeline/gujarati-readability-classification/data"
    output_dir = "/root/sarvam/akshar-experiments-pipeline/gujarati-readability-classification/augraphy_experiments/pdf_right_half_images"
    
    print("🚀 PDF Right Half Extractor")
    print(f"📁 Data directory: {data_dir}")
    print(f"💾 Output directory: {output_dir}")
    print("🎯 Process: PDF → Page Images → Right Half Crop")
    print("📐 Extracting right half of actual PDF pages")
    
    extractor = PDFRightHalfExtractor(data_dir, output_dir)
